...
var Methods = {
   ...
   */
  handleSelectRow: function handleSelectRow(_ref6, value, isForce) {
    var _this33 = this;
    var row = _ref6.row;
    var selection = this.selection,
      afterFullData = this.afterFullData,
    ...
    if (checkField) {
      if (treeConfig && !checkStrictly) {
        if (value === -1) {
          ...
          _xeUtils.default.eachTree([row], function (item) {
            if (_this33.eqRow(item, row) || isForce || !checkMethod || checkMethod({
              row: item
            })) {
        ...
        if (matchObj && matchObj.parent) {
          var parentStatus;
          var vItems = !isForce && checkMethod ? matchObj.items.filter(function (item) {
            return checkMethod({
              row: item
            });
          }) : matchObj.items;
      ...
      if (treeConfig && !checkStrictly) {
        ...
        if (_matchObj && _matchObj.parent) {
          var _parentStatus;
          var _vItems = !isForce && checkMethod ? _matchObj.items.filter(function (item) {
            return checkMethod({
              row: item
            });
          }) : _matchObj.items;
        ...
        if (childRecords) {
          return _this48.loadTreeChildren(row, childRecords).then(function (childRows) {
            ...
            if (!checkStrictly && _this48.isCheckedByCheckboxRow(row)) {
              _this48.handleCheckedCheckboxRow(childRows, true, true);
            }
